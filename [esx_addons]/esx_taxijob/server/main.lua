...
AddEventHandler('esx_taxijob:success', function(distanceDrive)
    ...
    if xPlayer.job.name == 'taxi' then
        if not lastPlayerSuccess[source] or timeNow - lastPlayerSuccess[source] > 5 then
			...
			local shouldPay = distanceDrive/1000*Config.perKMPay
			local tip = math.random(Config.NPCJobEarnings.min, Config.NPCJobEarnings.max)
			local total = ESX.Math.Round(shouldPay + tip)
			
			print("xplogdistanceDrive",distanceDrive,"->shouldpay",shouldPay,"tip",tip,"total",total)
...
ESX.RegisterServerCallback("esx_taxijob:SpawnVehicle", function(source, cb, model , props)
    ...
    ESX.OneSync.SpawnVehicle(joaat(model), SpawnPoint, Config.Zones.VehicleSpawnPoint.Heading, props, function(netId)
		...
		TriggerClientEvent('xp:client:HandleAllowVehicle',source, {plate = props.plate,isNeedLicensce = true,isReg = true})
