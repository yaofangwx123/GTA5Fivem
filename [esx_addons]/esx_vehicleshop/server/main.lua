...
function SQLVehiclesAndCategories()
	...
	vehicles = MySQL.query.await("SELECT vehicles.*, vehicle_categories.label AS categoryLabel FROM vehicles JOIN vehicle_categories ON vehicles.category = vehicle_categories.name where vehicles.category not in ('sportsclassics' ,'offroad', 'muscle' ,'coupes' ,'compacts')")
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function(source, cb, model, plate)
	...
	if modelPrice and xPlayer.getMoney() >= modelPrice then
		local position = xPlayer.removeMoney(modelPrice, "Vehicle Purchase")
		local coords = xPlayer.getCoords(true)
		MySQL.insert('INSERT INTO owned_vehicles (owner, plate, vehicle,position) VALUES (?, ?, ?,?)', {xPlayer.identifier, plate, json.encode({model = joaat(model), plate = plate}),json.encode({x = coords.x,y = coords.y,z = coords.z,heading = 0})}, function(rowsChanged)
			xPlayer.showNotification(TranslateCap('vehicle_belongs', plate))
			ESX.OneSync.SpawnVehicle(joaat(model), Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading,{plate = plate}, function(netId)
				Wait(100)
				local vehicle = NetworkGetEntityFromNetworkId(netId)
				Wait(300)
				TaskEnterVehicle(GetPlayerPed(source), vehicle, 10000,-1,2.0,1,0)
				cb(true,netId)
			end)
...
ESX.RegisterServerCallback('esx_vehicleshop:sellMyVehicle', function(source, cb,plate,money)
	MySQL.query('DELETE FROM owned_vehicles WHERE plate = ?', {plate}, function(result)
		if(result.affectedRows ~= 0 ) then
			print("xplog-esx_vehicleshop",ESX.DumpTable(result))
			local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_vehicleshop:repairMyVehicle', function(source, cb,money)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_vehicleshop:delVehicle', function(source,cb, plate)
	MySQL.query('DELETE FROM owned_vehicles WHERE plate = ?', {plate}, function(result)
		if(result.affectedRows ~= 0 ) then
			print("xplog-delVehicle",ESX.DumpTable(result))
