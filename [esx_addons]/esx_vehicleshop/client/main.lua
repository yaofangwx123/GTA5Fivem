...
RegisterNetEvent('esx_vehicleshop:updateVehiclesAndCategories', function(vehicles, categories, vehiclesByModel)
	...
	--print("sx_vehicleshop:updateVehiclesAndCategories---")
...
function OpenSellMyCarMenu()
	--print("allmycar",ESX.DumpTable(CurrentActionData.vehicles),"---------",ESX.DumpTable(vehiclesByCategory))
	if(CurrentActionData.vehicles == nil or #CurrentActionData.vehicles == 0) then
		ESX.ShowNotification(TranslateCap("sell_mycar_nocar"), 'error', 3000)
	return end
	ESX.UI.Menu.CloseAll()
	--print("curData",ESX.DumpTable(vehiclesByCategory))
	local elements = {}
	for i =1,#CurrentActionData.vehicles do
		local isMatch = false
		for k,v in pairs(vehiclesByCategory) do
			for j = 1,#v do
				
				if(joaat(v[j].model) == CurrentActionData.vehicles[i].vehicle.model and CurrentActionData.vehicles[i].properties ~= nil) then
				
					if(math.isnan(CurrentActionData.vehicles[i].properties.bodyHealth) or CurrentActionData.vehicles[i].properties.bodyHealth < 0) then
						CurrentActionData.vehicles[i].properties.bodyHealth = 0
					end
					
					if(math.isnan(CurrentActionData.vehicles[i].properties.engineHealth) or CurrentActionData.vehicles[i].properties.engineHealth < 0) then
						CurrentActionData.vehicles[i].properties.engineHealth = 0
					end
					
					if(math.isnan(CurrentActionData.vehicles[i].properties.tankHealth) or CurrentActionData.vehicles[i].properties.tankHealth < 0) then
						CurrentActionData.vehicles[i].properties.tankHealth = 0
					end
					
					local price = math.round(((CurrentActionData.vehicles[i].properties.bodyHealth + 
					CurrentActionData.vehicles[i].properties.tankHealth + 
					CurrentActionData.vehicles[i].properties.engineHealth)/3000.0)*v[j].price * 0.4)
				
					--print(CurrentActionData.vehicles[i].plate,price,math.round(price))
					
					if(price <= 0) then
						price = 1
					end
					isMatch = true
					print("mycarmode",CurrentActionData.vehicles[i].vehicle.model,joaat(v[j].model),v[j].model,((CurrentActionData.vehicles[i].properties.bodyHealth + 
					CurrentActionData.vehicles[i].properties.tankHealth)))
				
					if(price >=0 and price <= v[j].price) then
						table.insert(elements,{label = ('%s <span style="color:yellow;">$%s</span>'):format(CurrentActionData.vehicles[i].plate,price),value = CurrentActionData.vehicles[i].plate,price = price})
					end
		...
		if(not isMatch) then
			table.insert(elements,{label = ('%s <span style="color:yellow;">$%s</span>'):format(CurrentActionData.vehicles[i].plate,1),value = CurrentActionData.vehicles[i].plate,price = 1})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_sellMycars',{
			title = TranslateCap('sell_mycar_title'),
			align = 'top-left',
			elements = elements
		},function(data,menu)
		...
		ESX.UI.Menu.CloseAll()
		local plate = ESX.Trim(data.current.value)
		ESX.TriggerServerCallback("esx_vehicleshop:sellMyVehicle",function(result)
		
		if(result) then 
			ESX.ShowNotification(TranslateCap("sell_mycar_sell_sucess"), 'success', 3000)
			TriggerEvent("esx_vehicleshop:sellVehicle",plate)
		else
			ESX.ShowNotification(TranslateCap("sell_mycar_sell_fail"), 'error', 3000)
		end
		
		end,plate,data.current.price)
...
function OpenRepairMyCarMenu()
	...
	local elements = {}
		...
		for k,v in pairs(vehiclesByCategory) do
			for j = 1,#v do
				...
				if(joaat(v[j].model) == CurrentActionData.vehicle.model and CurrentActionData.properties ~= nil) then
				
					if(math.isnan(CurrentActionData.properties.bodyHealth) or CurrentActionData.properties.bodyHealth < 0) then
						CurrentActionData.properties.bodyHealth = 0
					end
					
					if(math.isnan(CurrentActionData.properties.engineHealth) or CurrentActionData.properties.engineHealth < 0) then
						CurrentActionData.properties.engineHealth = 0
					end
					
					if(math.isnan(CurrentActionData.properties.tankHealth) or CurrentActionData.properties.tankHealth < 0) then
						CurrentActionData.properties.tankHealth = 0
					end
				
					local price = ESX.Math.Round((v[j].price - ((CurrentActionData.properties.bodyHealth + 
					CurrentActionData.properties.tankHealth + 
					CurrentActionData.properties.engineHealth)/3000.0)*v[j].price)*0.6,1)
				
					--print(CurrentActionData.plate,price,math.round(price))
					...
					if(price >0 and price <= v[j].price) then
						table.insert(elements,{label = ('%s <span style="color:yellow;">$%s</span>'):format(CurrentActionData.properties.plate,price),value = CurrentActionData.plate,price = price})
					end
	...
	if(#elements == 0) then
		ESX.ShowNotification(TranslateCap("sell_repairmycar_nocar"), 'error', 3000)
		return
	end
	
	
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_repairMycars',{
			title = TranslateCap('sell_reparemycar_title'),
			align = 'center',
			elements = elements
		},function(data,menu)
		...
		ESX.TriggerServerCallback("esx_vehicleshop:repairMyVehicle",function(result)
		
		if(not result)  then
			ESX.ShowNotification(TranslateCap("not_enough_money"), 'error', 3000)
			return
		else 
				print(CurrentActionData.vehicle.id)
				
				CurrentActionData.properties.engineHealth = 1000.0
				CurrentActionData.properties.bodyHealth = 1000.0
				CurrentActionData.properties.tankHealth = 1000.0
				SetVehicleFixed(CurrentActionData.vehicle.id)
				ESX.Game.SetVehicleProperties(CurrentActionData.vehicle.id,CurrentActionData.properties)
				--SetPlaneEngineHealth(CurrentActionData.vehicle.id,1000 + 0.0)
				
				--SetVehicleBodyHealth(CurrentActionData.vehicle.id,1000 + 0.0)
				
				--SetVehiclePetrolTankHealth(CurrentActionData.vehicle.id,1000 + 0.0)
				ESX.ShowNotification(TranslateCap("sell_repairmycar_ok"), 'success', 3000)		
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					...
					ESX.TriggerServerCallback('esx_vehicleshop:buyVehicle', function(success,netId)
						if success then
							IsInShopMenu = false
							menu2.close()
							menu.close()
							...
							--告诉xp，车辆已买，需要重新加载车辆
							TriggerEvent("esx:buyVehicle",netId)
							
										--更新坐标，车的属性
							--[[RegisterNetEvent('esx:enteredVehicle', function(vehicle,plate, seat, displayName, netId)
							print('esx:enteredVehicle-buycar', 'vehicle', vehicle, 'plate', plate, 'seat', seat, 'displayName', displayName, 'netId', netId)
							local properties = ESX.Game.GetVehicleProperties(vehicle)
							print("properties",ESX.DumpTable(properties))
							
								local coords = GetEntityCoords(vehicle)
								local heading = GetEntityHeading(vehicle)
								
								
								TriggerServerEvent("esx_garage:updateVehicleOwner",plate,coords,heading,properties)
...
function hasEnteredMarker(zone)
	...
	elseif zone == 'ResellVehicle' then
		...
		ESX.TriggerServerCallback("esx_garage:getAllOwnerVehicles",function(result)
		    ...
		    CurrentAction     = 'sell_mycar'
			CurrentActionMsg  = TranslateCap('sell_mycar')
			CurrentActionData = {vehicles = result}	
			
		end)
	elseif zone == 'RepairVehicle_1' or zone == 'RepairVehicle_2' then
			...
			if (IsPedInAnyVehicle(pedId, false)) then
			  ...
			  if(GetPedInVehicleSeat(vehicle,-1) == pedId) then
				CurrentAction     = 'repair_mycar'
				CurrentActionMsg  = TranslateCap('repair_mycar',GetVehicleNumberPlateText(vehicle))
				local pros = ESX.Game.GetVehicleProperties(vehicle)
				
				CurrentActionData = {properties = pros,vehicle = {model =  pros.model,id = vehicle}}
	...
	elseif zone == 'ResellVehicle2' then
		local playerPed = PlayerPedId()
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'reseller_menu' then
					OpenResellerMenu()
				elseif CurrentAction == 'sell_mycar' then
					OpenSellMyCarMenu()
				elseif CurrentAction == 'repair_mycar' then
					OpenRepairMyCarMenu()					
