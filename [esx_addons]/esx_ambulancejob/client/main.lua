...
function StartDeathTimer()
  ...
  CreateThread(function()
	...
	--xp add this
	if(not Config.bloodTimeOutEnable) then
	      local playerPed = PlayerPedId()
		  local coords = GetEntityCoords(playerPed)
...
function RespawnPed(ped, coords, heading)
  ...
  --xp add this
  if(Config.localRespawn) then
     local pedCoords = GetEntityCoords(ped)
     local formattedCoords = {x = ESX.Math.Round(pedCoords.x, 1), y = ESX.Math.Round(pedCoords.y, 1), z = ESX.Math.Round(pedCoords.z, 1)}
