...
AddEventHandler('esx:onPlayerDeath', function(data)
	local source = source
	 ...
	 --xp add
	  if(Config.respawnFineAccount ~=0 and (not data.killedByPlayer)) then
	  TriggerEvent("esx_banking:cutMoneyBusiness",source,Config.respawnFineAccount,"Hospital")
		 end
		--[[local xPlayer = ESX.GetPlayerFromId(source)
		local moneyBank = xPlayer.getAccount("bank").money
		local moneyCrash = xPlayer.getMoney()
		print("moneyBank",moneyBank,"moneyCrash",moneyCrash)
		--local causeSource = GetPedSourceOfDeath(GetPlayerPed(source))
		local moneyFromCrash = 0
		
		local bankPay,crashPay = 0,0
		if(moneyBank >= Config.respawnFineAccount) then
			bankPay =  Config.respawnFineAccount
		else
			bankPay = moneyBank
			crashPay = Config.respawnFineAccount - moneyBank
		...
		if(moneyBank ~= 0) then
			local dataType = {withdrawonly = Config.respawnFineAccount,label = "Hospital"}
			TriggerEvent("esx_banking:doingType",source,dataType)
			
		end
		
		if(crashPay ~= 0) then
			xPlayer.removeAccountMoney("money",crashPay,"Hospital")
		end]]
		
		
	
		--print(ESX.DumpTable(causeSource),"data",ESX.DumpTable(data))
...
if Config.EarlyRespawnFine then
	...
	AddEventHandler('esx_ambulancejob:payFine', function()
		...
		--xp add
		if(Config.respawnFineAccount ~= 0) then return end
