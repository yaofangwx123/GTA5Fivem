...
-- 处理现金业务，
RegisterServerEvent('esx_banking:cutMoneyBusiness')
AddEventHandler('esx_banking:cutMoneyBusiness', function(source,money,reason)
		
		local xPlayer = ESX.GetPlayerFromId(source)
		local moneyBank = xPlayer.getAccount("bank").money
		local moneyCrash = xPlayer.getMoney()
		print("cutMoneyBusiness-->moneyBank",moneyBank,"moneyCrash",moneyCrash)
		--local causeSource = GetPedSourceOfDeath(GetPlayerPed(source))
		local moneyFromCrash = 0
		
		local bankPay,crashPay = 0,0
		if(moneyBank >= money) then
			bankPay =  money
		else
			bankPay = moneyBank
			crashPay =money - moneyBank
			if(moneyCrash < crashPay) then
				crashPay = moneyCrash
			end
		end

		if(moneyBank ~= 0) then
			local dataType = {withdrawonly = money,label = reason}
			TriggerEvent("esx_banking:doingType",source,dataType)
			
		end
		
		if(crashPay ~= 0) then
			xPlayer.removeAccountMoney("money",crashPay,reason)
		end
...
AddEventHandler('esx_banking:doingType', function(source,typeData)
	--print("esx_banking:doingType-1 ",type(source),"source ",source,ESX.DumpTable(typeData));
    if source == nil then return end
    if (typeData == nil) then return end

	print("esx_banking:doingType ",type(source),"source ",source,ESX.DumpTable(typeData));
    ...
    local amount
	print(ESX.DumpTable(typeData))
    local key = get_key(typeData)

	if(typeData.label == nil) then
		typeData.label = key
	end
    ...
    elseif typeData.withdrawonly then
        amount = tonumber(typeData.withdrawonly)
    ...
    if amount == nil or (not typeData.pincode and amount <= 0) then
        ...
        elseif typeData.withdrawonly and bankMoney ~= nil and bankMoney >= 0 then
            -- withdraw
            if(amount > bankMoney) then
            	amount = bankMoney
            end
            BANK.Withdrawonly(amount, xPlayer)    
        elseif typeData.pincode then
            -- pincode
        ...
        if not typeData.pincode then
            BANK.LogTransaction(source,string.upper(typeData.label), string.upper(key), amount, bankMoney)
        end
...
BANK = {
    ...
    Withdraw = function(amount, xPlayer)
        xPlayer.addAccountMoney('money', amount,"WITHDRAW","bankcallback")
        xPlayer.removeAccountMoney('bank', amount,"WITHDRAW","bankcallback")
    end,
    Withdrawonly = function(amount, xPlayer)
        --xPlayer.addAccountMoney('money', amount,"WITHDRAW")
        xPlayer.removeAccountMoney('bank', amount,"WITHDRAWONLY","bankcallback")
    end,
    Deposit = function(amount, xPlayer)
        xPlayer.removeAccountMoney('money', amount,"DEPOSIT","bankcallback")
        xPlayer.addAccountMoney('bank', amount,"DEPOSIT","bankcallback");
    end,
    Transfer = function(xTarget, xPlayer, amount, key)
        ...
        xPlayer.removeAccountMoney('bank', amount,"TRANSFER","bankcallback")
        xTarget.addAccountMoney('bank', amount,"TRANSFER")
