  ...
  renderTransactions() {
    ...
    for (let i = 0; i < transactionData.slice(0, 3).length; i++) {
      let oneRow = `
			...
			transactionData[i].type == this.language.withdrawonly ||
...
class Utils {
  static dateFormat(date) {
    if (typeof date == "number") {
	  ...
	  if(lang === "zh-CN")
	  {
		  return newDate.getFullYear()+"年" +(newDate.getMonth()+1) + "月" + newDate.getDate()+"日"+ newDate.getHours()+"时"+ newDate.getMinutes()+"分";
	  }
      ...
      transData.map((trans) => {
        ...
        switch (trans.type) {
          case "WITHDRAW":
		    ...
		    break;
		  case "WITHDRAWONLY":
            trans.type = languageText.withdrawonly;
            break;
          case "DEPOSIT":
		  ...
		  switch (trans.label) {
          case "WITHDRAW":
            trans.label = languageText.withdraw;
            break;
		  case "HOSPITAL":
		    trans.label = languageText.withdraw_hospital;
		    break;
          case "DEPOSIT":
            trans.label = languageText.deposit;
            break;
          case "PAYCHECK":
            trans.label = languageText.paycheck;
            break;
		  	
          default:
            trans.type = languageText.unknow;
            break;
        }
  ...
  $(document).on("click", "#more_history", function () {
    ...
    $("#example").DataTable({
      ...
      createdRow: function (row, data, index) {
        ...
        } else if (parseType == lang.withdraw || parseType == lang.withdrawonly || parseType == lang.transfer) {
