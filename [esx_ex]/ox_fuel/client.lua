...
RegisterNetEvent('ox_fuel:server:addFuelComplete')
AddEventHandler('ox_fuel:server:addFuelComplete', function(netid,fuel)
	local vehicle = NetworkGetEntityFromNetworkId(netid)
	local state = Entity(vehicle).state
	setFuel(state,vehicle,fuel*1.0,false)
	TriggerEvent('ox_fuel:addFuelComplete', vehicle,fuel)
...
lib.onCache('seat', function(seat)
	...
	if seat == -1 then
		SetTimeout(0, function()
			...
			--state.fuel = GetVehicleFuelLevel(vehicle)

			local fuelTick = 0

			while cache.seat == -1 do
				local fuel = GetVehicleFuelLevel(vehicle)
				--print("xplog carFuel",vehicle,fuel)
				local newFuel = fuel

				if fuel > 0 then
					if GetIsVehicleEngineRunning(vehicle) then
						local usage = Config.rpmUsage[math.floor(GetVehicleCurrentRpm(vehicle) * 10) / 10] * 0.1
