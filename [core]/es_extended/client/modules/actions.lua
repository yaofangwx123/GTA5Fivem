    ...
    while true do
        ...
        elseif isInVehicle then
            if not IsPedInAnyVehicle(playerPed, false) or IsPlayerDead(PlayerId()) then
				...
				local seat = GetPedVehicleSeat(playerPed, current.vehicle)
				if(seat ~= current.seat) then
					current.seat = seat
					TriggerEvent('esx:vehicleSeatChange', current.vehicle, current.plate, current.seat, current.displayName, current.netId)
				end
